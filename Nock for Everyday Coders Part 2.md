...
1. transform the subject using formula `c`. The transformed subject should have a formula somewhere in its memory tree.
2. grab the code chunk/formula from the new subject created in step ***(1)***. It must be located at memory slot `b` (important: `b` isn't evaluated as a formula: it's **just** an atom/number.)
3. run the formula grabbed in step ***(2)*** against the new subject from step ***(1)***

The fact that the above description has the words "transform the subject" tells us right away that it's syntactic sugar for opcode `2`, and that's indeed what we see in the pseudocode:
